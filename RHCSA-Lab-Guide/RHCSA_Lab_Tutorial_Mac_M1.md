...
## MacBook M1 (Apple Silicon) + UTM Virtualize + Rocky Linux 9.8 aarch64
...
>
> **Platform**: MacBook M1 · UTM (mode Virtualize) · Rocky Linux 9.8 **aarch64** (ARM64)
...
2. [Tahap 1 — Setup UTM & Instalasi Rocky Linux](#2-tahap-1--setup-utm--instalasi-rocky-linux)
...
> [!IMPORTANT]
> **Minimum RAM MacBook**: 16 GB (agar bisa menjalankan 3 VM + macOS bersamaan).
...
Network    : Shared Network (UTM default, mendapat IP otomatis)
ISO        : Rocky-9.8-aarch64-dvd.iso
Mode UTM   : Virtualize (BUKAN Emulate — native speed di M1)
```
...
### Download ISO Rocky Linux 9.8 (aarch64)

> [!IMPORTANT]
> MacBook M1 adalah chip **Apple Silicon (ARM)**. Kamu **WAJIB** download versi **aarch64**.
> Jangan download versi x86_64 — itu untuk Intel dan akan berjalan sangat lambat di M1 (emulasi).

1. Buka browser → [https://rockylinux.org/download](https://rockylinux.org/download)
2. Pilih **Rocky Linux 9.8**
3. Pilih arsitektur: **aarch64** ← yang ini!
4. Download file **DVD ISO** (~9-10 GB) — berisi semua package offline
5. Simpan file `Rocky-9.8-aarch64-dvd.iso` di folder yang mudah ditemukan (misal `~/Downloads`)

---

## 2. Tahap 1 — Setup UTM & Instalasi Rocky Linux

### 2.1 Install UTM di MacBook

```
📍 Posisi: Di macOS (bukan di dalam VM)
👤 User: User biasa macOS kamu
```

1. Buka **App Store** di MacBook
2. Search "**UTM**"
3. Klik **Get** → Install (gratis)
4. Atau download dari: [https://mac.getutm.app](https://mac.getutm.app)

### 2.2 Membuat VM-1 (`server1.lab.local`)

```
📍 Posisi: Di aplikasi UTM (macOS)
👤 User: User biasa macOS kamu
```
...
3. Pilih **"Linux"**
4. **Boot ISO Image** → klik **Browse** → pilih file `Rocky-9.8-aarch64-dvd.iso`
5. Konfigurasi Hardware:
   - **Memory**: `2048` MB
   - **CPU Cores**: `2`
6. Storage:
   - **Size**: `20` GB
7. Shared Directory: skip (biarkan kosong dulu)
8. Summary:
   - Name: `server1-lab`
   - Klik **Save**
...
1. Klik VM `server1-lab` → klik ikon **⚙️ Settings** (kanan atas)
2. Scroll ke bagian **Drives**
3. Klik **"New Drive"**
   - Interface: **VirtIO**
   - Size: **5 GB**
4. Klik **Save**
...
1. Di Settings VM → bagian **Network**
2. Network Mode: **Shared Network** (ini seperti NAT — VM dapat akses internet dan bisa komunikasi antar VM)
3. Klik **Save**

### 2.3 Instalasi Rocky Linux 9.8 (VM-1)

```
📍 Posisi: Di dalam jendela VM UTM (booting dari ISO)
...
> Di UTM → Settings VM → Drives → klik drive CD/DVD → **Clear** (hapus ISO)

### 2.4 Login Pertama Kali ke VM-1

```
📍 Posisi: Di dalam VM (console UTM)
...
Kernel 5.14.0-503.x.el9.aarch64 on an aarch64
...
| Parameter | VM-2 | VM-3 |
|---|---|---|
| Name di UTM | `server2-lab` | `server3-lab` |
| RAM | 2048 MB | 1536 MB |
| CPU | 2 | 1 |
...
**Penjelasan**: `lsblk` = **l**i**s**t **bl**oc**k** devices — menampilkan semua disk dan partisi dalam bentuk tree. Disk 5 GB tambahan akan muncul sebagai `vdb` (atau `sdb`) tanpa partisi.

Output contoh:
```
NAME          MAJ:MIN RM  SIZE RO TYPE MOUNTPOINTS
vda             0:0    0   20G  0 disk 
├─vda1          0:1    0    1G  0 part /boot
└─vda2          0:2    0   19G  0 part 
  ├─rl-root   0:0    0   17G  0 lvm  /
  └─rl-swap   0:1    0    2G  0 lvm  [SWAP]
vdb             0:16   0    5G  0 disk          ← disk tambahan, belum dipartisi!
```
...
> [!CAUTION]
> Latihan partisi ini dilakukan pada **disk kedua (`/dev/vdb`)** yang kosong. **JANGAN** partisi disk utama (`/dev/vda`) karena bisa merusak OS!
...
```bash
vgcreate vg_data /dev/vdb1 /dev/vdb2
```
...
- File `linux-arm64` = binary untuk arsitektur ARM64/aarch64 (sesuai Rocky Linux kita di M1)
...
#### LAB: Provisioning Storage (dari disk /dev/vdb)
```bash
pvcreate /dev/vdb
...
12. **Task 12**: `setfacl -d -m g:devops:rwx /shared/devdata`
...
| `fdisk /dev/vdb` | root | Partisi disk | VM-1 |
